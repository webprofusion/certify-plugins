...
namespace Certify.Providers.DeploymentTasks
{
    public class UpdatePortBinding : IDeploymentTaskProvider
    {
        ...
        public async Task<List<ActionResult>> Validate(DeploymentTaskExecutionParams execParams)
        {
            ...
            var guid = execParams.Settings.Parameters.FirstOrDefault(p => p.Key == "appid")?.Value;

            if (!string.IsNullOrWhiteSpace(guid) && (!Guid.TryParse(guid, out var parsedGuid) || guid.Contains("{")))
            {
                results.Add(new ActionResult("Invalid AppID (GUID). Don't include brackets and leave blank to use an autogenerated guid.", false));
            }
